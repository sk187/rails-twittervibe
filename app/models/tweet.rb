...
class Tweet < ActiveRecord::Base
  belongs_to :user
	...
	def self.add_tweets_to_database(tweets, id)
		...
		tweets.each do |t|
		 	if Tweet.where(tweetid: t.id).exists?
				...
				score = calculate_afinn_score(t)
				datearray = calculate_datearray(t)
				datetime = calculate_datetime(t)
				create_tweet(t, score, datearray, id, datetime)
	...
	def self.calculate_afinn_score(t)
		#get tweet body
		str = t.text.dup 
	...
	def self.calculate_datearray(t)
	...
	def self.create_tweet(t, score, datearray, id, datetime)
		Tweet.create(
			user_id: id, 
			text: t.text, 
			tweetid: t.id,
	...
	def self.create_day_score (id)
		...
		ordered_latest_tweets.each do |tweet|	
			...
			@reference_date = 
				...
				@reference_date)
				# raise
			date = calculate_date(tweet)
			
			if @reference_date == date
				tweet_score = tweet.sentimentscore.to_i 
				@minscore = calculate_min(tweet_score, @minscore)
				@maxscore = calculate_max(tweet_score, @maxscore)
				@sum = calculate_daily_sum(tweet_score, @sum)
				@num_of_tweets +=1
			else
				avgscore = calculate_avg(@sum)
				datetime = tweet.datetime
				entry = Scoreboard.find_by(user_id: id, 
					reference_date: @reference_date)
				if entry.nil?
					create_new_entry(@reference_date, datetime, 
						@minscore, @maxscore, id, @avgscore)
				else
					update_entry(entry)		
				end
				@reference_date = recalculate_reference_date(tweet)
				reset_values
				tweet_score = tweet.sentimentscore.to_i 
				@minscore = calculate_min(tweet_score, @minscore)
				@maxscore = calculate_max(tweet_score, @maxscore)
	...
	def self.calculate_first_reference_date(tweet, 
		tweets_processed, reference_date)
	...
	def self.recalculate_reference_date(tweet)
		tweet.year+tweet.month+tweet.day
	end

	def self.calculate_date(tweet)
		date = tweet.year+tweet.month+tweet.day
	end

	def self.calculate_min(tweet_score, minscore)
		if tweet_score <= @minscore
			@minscore = tweet_score
		else
			@minscore = @minscore
		end
	end

	def self.calculate_max(tweet_score, maxscore)
		if tweet_score >= @maxscore
	...
	def self.calculate_daily_sum(tweet_score, sum)
		sum_int = sum.to_i
		@sum = sum_int + tweet_score
	end

	def self.calculate_avg(sum)
		sum_int = sum.to_i
		if sum_int == 0
			@avgscore = 0
		else
	...
	def self.create_new_entry (reference_date, datetime, minscore,
		maxscore, id, avgscore)
		Scoreboard.create(
			reference_date: reference_date,
			datetime: datetime,
			minscore: minscore,
